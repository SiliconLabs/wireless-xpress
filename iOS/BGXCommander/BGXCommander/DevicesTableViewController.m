...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  NSDictionary * deviceRecord = [self.devices objectAtIndex:indexPath.row];

  NSNumber * rssi = [deviceRecord objectForKey:@"RSSI"];

#if TARGET_IPHONE_SIMULATOR
  NSString * name = [deviceRecord objectForKey:@"name"];
  NSString * uuid = [deviceRecord objectForKey:@"UUID"];
#else
  CBPeripheral * iPeripheral = [deviceRecord objectForKey:@"peripheral"];

  NSString * name = [[deviceRecord objectForKey:@"adv"] objectForKey:@"kCBAdvDataLocalName"];
  NSString * uuid = [iPeripheral.identifier.UUIDString copy];
#endif
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  [self performSegueWithIdentifier:@"pushDetails" sender:self];
#else
  NSDictionary * iDict = [self.devices objectAtIndex:indexPath.row];
  NSString * advertisedName = [[iDict objectForKey:@"adv"] objectForKey:@"kCBAdvDataLocalName"];
  self.selectedPeripheral = [iDict objectForKey:@"peripheral"];

  [self showConnectingWindow];
  if (![[AppDelegate sharedAppDelegate].bgxManager connectToDevice: advertisedName]) {
    [self cancelConnectingAction: nil];
  }
#endif
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  ...
  NSDictionary * iDict = [self.devices objectAtIndex:self.tableView.indexPathForSelectedRow.row];

  CBPeripheral * iPeripheral = [iDict objectForKey:@"peripheral"];

  if (self.selectedPeripheral != iPeripheral) {
    NSLog(@"WARNING: selected peripheral doesn't match.");
  }
  ddvc.title = iPeripheral.name;
}
...
- (IBAction)cancelConnectingAction:(id)sender
{
  if (self.connectingWindow) {
    NSAssert(self.selectedPeripheral, @"No selected peripheral");
